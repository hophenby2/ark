...
| 事件楼层资格 | 明确楼层已接线 | 250 条 canonical 注解按主题、节点类别和 171 条显式楼层过滤；RO3 深埋迷境专属项排除；不再回退全主题池 | 补模式、历史、完整结局条件、权重与 RO3-5 效果 |
| 区域与结局资料 | 核心布局已接线 | 36 条核心区域长度/分支上限及逐列审核约束进入生成器；22 条终点用于 canonical Boss 和终止层 | 建立有序 EndingRoute 状态机；特殊/平面区域单独建模 |
| 地图生成 | 核心区域约束生成，仍有近似 | 同 seed 可复现、相邻列可达、无空 stage Boss；问号列权重和连线概率仍为近似 | 扩大固定种子性质测试并补完整隐藏路线 |
...
- 新增完整矩阵、边界、规则适配和 handler 事务测试；当前 roguelike 相关测试为 **101 项通过**。
...
- 默认结局在 zone 5 正常结束；22 条结局终点和 Boss 映射可供生成器查询，但生产流程尚不会更新 `toEnding`。完整结局触发、有序区域序列和同层 Boss 替换仍缺 EndingRoute 状态机，不能把所有路线简化为 `zone + 1`。
- Boss、传送/特殊区域、RO4 旗帜、紧急额外掉落及藏品/主题乘区仍无可靠完整规则；当前不会回退到普通/紧急矩阵。
- 奖励外形已按 `2.7.51` IL2CPP 字段约束实现，但仍缺同版本真实请求/响应 fixture。
- 171 条明确楼层已接线，但模式、历史、完整结局条件、权重及 RO3-5 choice 效果仍为空，4 条特殊节点/变体继续 quarantine；楼层资格不等于事件完整还原。
- 36 条核心线性摘要和 22 条终点映射已接线；入口条件 AST、完整有序路线、特殊/平面区域、事件替换与结算顺序仍为空，6 条记录继续 quarantine。
