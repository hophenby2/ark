...
用户结论：`gold为独立的reward，而且可以选择不领，经验以外的其他奖励都可以选择不领直接结束奖励阶段，只有经验会直接结算；普通和紧急战斗都是掉一张随机职业的券；若为3/5层则为两张随机职业的券二选一；紧急会多掉一个随机藏品`
...
用户结论：`奖励内容是伴随关卡而不是伴随层数；关卡生成逻辑为：ro2的特殊区域只会生成5/6层的关卡，ro3的特殊区域会生成当前层的关卡，ro4的特殊区域会生成当前层和下一层的关卡，节点排布看prts的区域部分，展开之后有排布；除ro3外，特殊区域所生成的事件池与进入特殊区域前一致；ro3特殊区域有专门的事件池，请预留该类型事件的tag供人工标记`
...
用户结论：`旗帜是不是prts的印象重建章节，你看看那里的描述，如果是就按那个来`
...
用户结论：`经验和金币乘算（*1.1*1.1），其他的加算，看prts；额外掉落分别独立判断并计算`
...
用户结论：`先把重名事件命名为xx123这种，正常实现，后续再改`
...
用户结论：`看prts的区域条目中对应区域的进入条件；奖励都按第六层来；先在本地建立矩阵确定后后面再改`
...
用户结论：`先看prts的区域章节中节点排布逻辑，总结进文档，后面再改`
...
用户结论：`a战斗后50%概率抽取，仅抽取本层本节点后续存在的节点对应的剧目，对应节点开始时触发，权重均一致，未命中不消耗，换层会重置
bcde看prts，先制定描述，先不要改`
